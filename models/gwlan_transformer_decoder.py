...
class GWLANTransformerDecoder(TransformerDecoderBase):
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            if self.training:
                # self_attn_mask: (seq_len, seq_len) 因此，只能使用batch size为1的情况
                # tmp = prev_output_tokens.eq(50000)
                # tmp = prev_output_tokens.eq(self.dictionary.mask())  # TODO: （1）修改成tgt_dict所特有的mask()!因为英文和中文的词典大小不一样， 否则性能会特别差。（2）修改成 (bsz * num_heads, tgt_len, src_len)的形式
                # assert self.dictionary.mask() == 60000
                # self_attn_mask_bool = tmp[0].unsqueeze(0).expand(slen, slen)
                # self_attn_mask = torch.zeros(slen, slen).to(prev_output_tokens.device).masked_fill(self_attn_mask_bool, -float('inf'))
                # self_attn_mask = None  # as GWLAN's setting, we should masked self-attention, just for check
