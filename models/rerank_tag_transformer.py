...
@register_model("rerank_tag_transformer")
class RerankTagTransformerModel(TransformerModelBase):
    """
    ...
    def forward(
        ...
        generator_bi_context_position = None,
        generator_bi_context_input_tokens = None,
        generator_target_position = None,   
        **kwargs
    ):
        """
            ...
            if hasattr(self.cfg, "k_folds") and self.cfg.k_folds > 1:
                ...
                with torch.no_grad():
                    baseline_net_output = self.baseline(
                        src_tokens,
                        src_lengths,
                        generator_bi_context_input_tokens,
                        generator_bi_context_position,
                    )
        elif mode == "infer":
            # 首先运行self.baseline得到baseline的输出
            self.baseline.eval()
        ...
        logits = logits.gather(1, generator_target_position.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, logits.size(-1))).squeeze(1)
        ...
        if self.cfg.dynamic_negative_sampling == False:
            ...
            if mode == "fit":
                ...
                # p: (bsz, 20), index: (bsz, top_k-1)
                p = torch.full_like( input=top_k_indices , fill_value=(1.0 / top_k_indices.size(1)), dtype=torch.float)
                index = p.multinomial(num_samples=self.cfg.top_k-1, replacement=True)
...
@register_model_architecture("rerank_tag_transformer", "rerank_tag_transformer")
...
@register_model_architecture("rerank_tag_transformer", "rerank_tag_transformer_big")
