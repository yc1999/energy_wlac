...
@register_model("gwlan_transformer")
class GWLANTransformerModel(TransformerModelBase):
    """
    ...
    def __init__(self, args, encoder, decoder):
        ...
        if hasattr(cfg, "mlm_model_path") and cfg.mlm_model_path is not None:  
            self.load_mlm_model(cfg.mlm_model_path)

    def load_mlm_model(self, path):
        """
        Given the path to the mlm model, load the model and return it.
        Make sure that this code doesn't effect previous code.
        """
        from fairseq.checkpoint_utils import load_checkpoint_to_cpu
        from fairseq import tasks

        logger.info("Loading mlm model from {}.".format(path))
        state = load_checkpoint_to_cpu(path)
        cfg = state["cfg"]
        task = tasks.setup_task(cfg.task)
        mlm_model = task.build_model(cfg.model, from_checkpoint=True)
        mlm_model.load_state_dict(state["model"], strict=True, model_cfg=cfg.model)
        mlm_model.eval()
        
        # cut down the gradient
        for param in mlm_model.parameters():
            param.requires_grad = False

        logger.info("After loading mlm model, we initilaize generator model from {}".format(path))
        state = load_checkpoint_to_cpu(path)
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        cfg = TransformerConfig.from_namespace(args)
        return GWLANTransformerDecoder(
            cfg,
            tgt_dict,
            embed_tokens,
...
@register_model_architecture("gwlan_transformer", "gwlan_transformer")
...
@register_model_architecture("gwlan_transformer", "gwlan_transformer_big")
...
@register_model("btba_transformer")
class BTBATransformerModel(GWLANTransformerModel):
    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        cfg = TransformerConfig.from_namespace(args)
        return BTBATransformerDecoder(
            cfg,
            tgt_dict,
            embed_tokens,
            no_encoder_attn=cfg.no_cross_attention,
...
@register_model_architecture("btba_transformer", "btba_transformer")
def base_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
