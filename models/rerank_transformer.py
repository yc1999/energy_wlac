...
def read_synonyms(tgt_dict):
    ...
    if "/apdcephfs/share_916081/khalidcyang/project/gwlan-fairseq/" in dictionary_file:
        dictionary_file = dictionary_file.replace("/apdcephfs/share_916081/khalidcyang/project/gwlan-fairseq/", "/data/users/zhangjunlei/yangc/project/khalidcyang/wlac-fairseq/")
    # 建立词典
    symbols = dict()
    indices = dict()
...
@register_model("rerank_transformer")
class RerankTransformerModel(TransformerModelBase):
    """
    ...
    def load_baseline(self, path):
        ...
        if "/apdcephfs/share_916081/khalidcyang/project/gwlan-fairseq/" in path:
            path = path.replace("/apdcephfs/share_916081/khalidcyang/project/gwlan-fairseq/", "/data/users/zhangjunlei/yangc/project/khalidcyang/wlac-fairseq/")
        ...
        #         if hasattr(self.cfg, "reranker_embed_init_only") and self.cfg.reranker_embed_init_only == True:
        #             logger.info("Use pretrained MLM to initialize reranker's embed_tokens layer only.")
        #             new_dict = { k:v for k,v in pretrained_dict.items() if k in rerank_transformer_dict.keys() and "decoder.output_projection" not in k and "embed_tokens" in k }
        #         else:
    ...
    def get_masked_logits(self, type_masks, logits):
        logits = logits.masked_fill(type_masks, -float('inf'))
        return logits
        ...
        **kwargs
        ...
        if self.cfg.dynamic_negative_sampling == False:
            if mode == "fit":
                ...
                # if "ids" in kwargs:
                #     top_k_indices = []
                #     with open(kwargs["candidates_file"], "r") as f:
                #         output_dict = json.load(f)
                #     for id in kwargs["ids"]:
                #         top_k_indices.append([ tgt_dict.index(cand)  for cand in output_dict[id]["candidates"] ] )
                #     top_k_indices = torch.tensor(top_k_indices).to(src_tokens.device)
    ...
    def forward_save_attn(
        ...
        **kwargs
        ...
        top_k_indices = target.unsqueeze(1)
        ...
        #     # print("gather_decoder_out_logits: ", gather_decoder_out_logits.size())
        #     decoder_out = (gather_decoder_out_logits,) + decoder_out[1:]
        return (decoder_out + (target, top_k_indices, cross_entropy_target, prev_output_tokens) )
    ...
    def forward_combine(
        ...
        **kwargs
        ...
        # probs: (bsz, vocab_size)
        probs = self.get_masked_probs(type_masks, logits)
        logits = self.get_masked_logits(type_masks, logits)
        ...
        if self.cfg.dynamic_negative_sampling == False:
            if mode == "fit":
                ...
                generator_top_k_logits = sorted_logits[:, :self.cfg.top_k]
                # do softmax
                # print(top_k_indices.tolist())
                # print([ tgt_dict.symbols[index] for index in top_k_indices.tolist()[0] ])
                # print(top_k_probs.tolist()) 
        ...
        #     # print("gather_decoder_out_logits: ", gather_decoder_out_logits.size())
        #     decoder_out = (gather_decoder_out_logits,) + decoder_out[1:]
        return (decoder_out + (target, top_k_indices, cross_entropy_target, generator_top_k_logits) )
...
@register_model_architecture("rerank_transformer", "rerank_transformer")
...
@register_model_architecture("rerank_transformer", "rerank_transformer_big")
