...
class MLM:
    ...
    def __call__(self, x: torch.Tensor):
        full_mask = torch.rand( x.shape, ) < self.masking_prob
        
        for t in self.no_mask_tokens:
            full_mask &= x != t

        unchanged = full_mask & (torch.rand( x.shape, ) < self.no_change_prob)

        random_token_mask = full_mask & (torch.rand( x.shape, ) < self.randomize_prob)

        random_token_idx = torch.nonzero(random_token_mask, as_tuple=True)

        random_tokens = torch.randint(0, self.n_tokens, (len(random_token_idx[0]), ), )
...
def sample_types(tgt_tokens, tgt_dict, symbol2pinyin):
    """
    tgt_tokens: (mask_token_size, ) Tensor of target tokens.#TODO :中文是拼音要进行修改
    """
...
def collate(
    ...
    if samples[0].get("target", None) is not None:
        ...
        if samples[0].get("prev_output_tokens", None) is not None:
            ...
            # We do mask language modeling here.
            prev_output_tokens, target, is_mlm_applied = mlm(prev_output_tokens)

            # TODO: we should get the type masking for target words
            no_pad_target = target[ ~target.eq(tgt_dict.pad()) ]
            types = sample_types(no_pad_target, tgt_dict, symbol2pinyin)
            type_masks = get_type_masks(types, tgt_dict, symbol2pinyin, no_pad_target.tolist())
...
class GWLANPRerankDataset(FairseqDataset):
    """
    ...
    def __init__(
        ...
        self.split = split
        # MLM
        self.mlm = MLM(padding_token=self.tgt_dict.pad(),
                       mask_token=self.tgt_dict.mask(),
                       no_mask_tokens=[self.tgt_dict.bos(), self.tgt_dict.eos(), self.tgt_dict.unk()],
                       n_tokens = len(self.tgt_dict.symbols) )
