...
def add_word_suggestion_sample_v4(seqs, sugst_type, mask_idx, pad_idx, eos_idx, tgt_dict):
    """
    ...
    seqs_new = convert_list_to_tensor(seqs_new, pad_idx)
    position_ids = convert_list_to_tensor(position_ids, pad_idx)
    anchors = torch.tensor(anchors, dtype=torch.int64)

    return seqs_new, position_ids, anchors
    ...
    if samples[0].get("target", None) is not None:
        ...
        if samples[0].get("prev_output_tokens", None) is not None:
            ...
            if suggestion_type == "bi_context":
                ...
                # get input tokens , position ids and target position
                imt_input_tokens, imt_position, target_position = add_word_suggestion_sample_v4(
                ...
                # resort imt_input_tokens, imt_postion, target, target_position, type_masks 
                imt_input_tokens = imt_input_tokens.index_select(0, sort_order)
                imt_position = imt_position.index_select(0, sort_order)
                target_position = target_position.index_select(0, sort_order)
                target = target.index_select(0, sort_order)
                type_masks = type_masks.index_select(0, sort_order)
...
class GWLANRerankTraditionalDataset(FairseqDataset):
    """
