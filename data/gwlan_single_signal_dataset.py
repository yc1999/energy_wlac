    ...
    if samples[0].get("target", None) is not None:
        ...
        if samples[0].get("prev_output_tokens", None) is not None:
            ...
            if suggestion_type == "bi_context":
                ...
                bi_context_input_tokens, bi_context_position, pos = add_word_suggestion_sample(prev_output_tokens, "bi_context", mask_idx, pad_idx)
                # for target, indeed, we should mask what is not mask token.
                # taget_mask : (bsz, tgt_len)
                # try:
                target_mask = torch.full_like(input=bi_context_input_tokens, fill_value=True, dtype=torch.bool)
                target_mask[:, pos] = False
...
class GWLANSingleSignalDataset(FairseqDataset):
    """
