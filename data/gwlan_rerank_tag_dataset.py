...
def gen_bi_context_mask_tag(tgt_len, pos):
    if tgt_len <= 1:
        # Ideally, this should not happen. Because the input sentence should have at least 2 tokens.
        print("tgt_len <= 1".center(100, "*"))
        assert False
    ...
    span = gen_bi_context_span_from_sent(tgt_len, pos)
    masked = [0] * (tgt_len+2)
    generator_masked = [0] * tgt_len
    # if span[1] == tgt_len-2:
        # print("here")
        # pass
    for word_id in range(span[0]+1, span[1]+2):   # span[0] is inclusive, span[1] is inclusive
        masked[word_id] = 1

    for word_id in range(span[0], span[1]+1):
        generator_masked[word_id] = 1
    return masked, generator_masked,span
...
def add_word_suggestion_sample_v4(input_matrix, sugst_type, mask_idx, pad_idx, tgt_dict):
    batch_size = input_matrix.size(0)
    # This includes the eos token
    target_lengths = torch.sum( input_matrix.ne(pad_idx).int(), dim=-1 )
    min_tgt_len = torch.min(target_lengths).item()
    ...
    input_matrix_new = input_matrix.clone().detach()
    input_matrix_insert_tag = []
    
    anchors = []
    generator_anchors = []
    
    tgt_mask_batch = []
    generator_tgt_mask_batch = []
    
    tgt_pos_batch = []
    generator_tgt_pos_batch = []
    ...
    if min_tgt_len > 1:
        for i in range(batch_size):
            ...
            if sugst_type == "bi_context":
                ...
                generator_anchors.append(anchor)
                # tgt_mask: (tgt_len + 2)
                tgt_mask, generator_tgt_mask, span = gen_bi_context_mask_tag(cur_tgt_len, anchor)
            ...
            if sugst_type == "bi_context": 
                new_tgt_mask = tgt_mask + [0] * max( input_matrix_new.size(1) - cur_tgt_len, 0 )
                generator_new_tgt_mask = generator_tgt_mask + [0] * max( input_matrix_new.size(1) - cur_tgt_len, 0 )

                generator_tgt_mask_batch.append(generator_new_tgt_mask)
                tgt_mask_batch.append(new_tgt_mask)
                
                tgt_pos_batch.append( get_pos_with_mask(new_tgt_mask, cur_tgt_len+2, pad_idx) )
                generator_tgt_pos_batch.append(  get_pos_with_mask(generator_new_tgt_mask, cur_tgt_len, pad_idx) )
            elif sugst_type == "prefix":
                new_tgt_mask = tgt_mask + [1] * (cur_tgt_len - len(tgt_mask))
                new_tgt_mask = new_tgt_mask + [0] * (max_tgt_len - len(new_tgt_mask))
                tgt_mask_batch.append(new_tgt_mask)
            ...
            # ! We need rewrite the input_matrix_new to new

            input = input_matrix_new[i,:].tolist()
            input.insert(span[0], tgt_dict.index("<s>"))
            input.insert(span[1]+2, tgt_dict.index("<s>"))
            for j in range(len(input)):
                if tgt_mask_batch[i][j]:
                    input[j] = mask_idx
            input_matrix_insert_tag.append(input)

        for i in range(batch_size):
            for j in range(max_tgt_len):
                if generator_tgt_mask_batch[i][j]:
                    input_matrix_new[i, j] = mask_idx

        input_matrix_insert_tag = input_matrix_new.new(input_matrix_insert_tag)
        tgt_pos_batch = input_matrix_new.new(tgt_pos_batch)
        anchors = input_matrix_new.new(anchors)
        generator_tgt_pos_batch = input_matrix_new.new(generator_tgt_pos_batch)
        generator_anchors = input_matrix_new.new(generator_anchors)
    
    return input_matrix_insert_tag, tgt_pos_batch, anchors, input_matrix_new, generator_tgt_pos_batch, generator_anchors 
            ...
            if suggestion_type == "bi_context":
                ...
                if v2 == False:
                    ...
                    if v3 == False and split == "train":
                        ...
                        bi_context_input_tokens, bi_context_position, target_position, generator_bi_context_input_tokens, generator_bi_context_position, generator_target_position  = add_word_suggestion_sample_v4(prev_output_tokens, "bi_context", mask_idx, pad_idx, tgt_dict)
                        target = target.gather(1, (target_position-1).unsqueeze(-1)).squeeze(-1)
                        types = sample_types(target, tgt_dict, symbol2pinyin)
                        type_masks = get_type_masks(types, tgt_dict, symbol2pinyin, target.tolist())
...
class GWLANRerankTagDataset(FairseqDataset):
    """
