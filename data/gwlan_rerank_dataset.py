    ...
    if min_tgt_len > 1:
        for i in range(batch_size):
            ...
            if sugst_type == "bi_context":
                ...
                tgt_mask = gen_ts_mask(cur_tgt_len, anchor)
                ...
                while len(tgt_dict.symbols[ input_matrix[i][anchor] ]) < len_limit and resample_cnt < 3:
                    ...
                    resample_cnt += 1
                anchors.append(anchor)
                tgt_mask = gen_prefix_mask(cur_tgt_len, anchor)
    ...
    if samples[0].get("target", None) is not None:
        ...
        if samples[0].get("prev_output_tokens", None) is not None:
            ...
            if suggestion_type == "bi_context":
                ...
                ts_input_tokens, ts_position, target_position = add_word_suggestion_sample_v2(prev_output_tokens, "ts", mask_idx, pad_idx, tgt_dict)
                target = target.gather(1, target_position.unsqueeze(-1)).squeeze(-1)
                types = sample_types(target, tgt_dict, symbol2pinyin)
                type_masks = get_type_masks(types, tgt_dict, symbol2pinyin, target.tolist())
                ...
                # get input tokens , position ids and target position
                imt_input_tokens, imt_position, target_position = add_word_suggestion_sample_v2(prev_output_tokens, "imt", mask_idx, pad_idx, tgt_dict)
                target = target.gather(1, target_position.unsqueeze(-1)).squeeze(-1)
                types = sample_types(target, tgt_dict, symbol2pinyin)
                type_masks = get_type_masks(types, tgt_dict, symbol2pinyin, target.tolist())
...
class GWLANRerankDataset(FairseqDataset):
    """
