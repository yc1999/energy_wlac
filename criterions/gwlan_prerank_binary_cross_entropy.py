...
@register_criterion(
    "gwlan_prerank_binary_cross_entropy", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class GWLANPRerankBinaryCrossEntropyCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if sample["suggestion_type"] == "bi_context":
            net_output = model(
                ...
                is_mlm_applied = sample["is_mlm_applied"],
    ...
    def get_logits_and_target(self, model, net_output, sample):
        ...
        return logits.view(-1), target.view(-1)
