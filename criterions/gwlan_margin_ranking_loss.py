...
from torch.nn import BCEWithLogitsLoss, CrossEntropyLoss, MarginRankingLoss
...
class LabelSmoothedCrossEntropyCriterionConfig(FairseqDataclass):
    ...
    margin: float = field(
        default=0.5,
        metadata={"help": "margin for the margin ranking loss"},
    )
...
@register_criterion(
    "gwlan_margin_ranking_loss", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class GWLANMarginRankingLoss(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
        ...
        self.margin_ranking_loss = MarginRankingLoss(margin=margin, reduction="sum")
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        # We use nn.MarginRankingLoss to calculate the loss.

        # First, we should reshape the logits from (bsz * top_k) to (bsz, top_k)
        logits = logits.view(-1, model.cfg.top_k)

        # Then, We apply sigmoid to the logits to get the probability.
        # (bsz, top_k) -> (bsz, top_k)
        probs = torch.sigmoid(logits)

        # Then, we get the ground-truth as x1 and the second largest as x2
        # x1 : (bsz)
        x1 = probs[:, 0]
        # x_with_ground_truth: (bsz, top_k-1)
        x_with_ground_truth = probs[:, 1:]
        # get largest element from x_with_ground_truth
        # x2: (bsz)
        x2 = x_with_ground_truth.max(1)[0]
        target = torch.full_like(x1, fill_value=1)  # will move to GPU automatically
