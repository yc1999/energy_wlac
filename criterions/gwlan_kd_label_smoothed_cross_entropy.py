...
class LabelSmoothedCrossEntropyCriterionConfig(FairseqDataclass):
    ...
    kd_alpha: float = field(
        default=0.0,
        metadata={"help": "alpha for knowledge distillation student network, \
                0 means no kd loss."}
    )
...
@register_criterion(
    "gwlan_kd_label_smoothed_cross_entropy", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class GWLANKDLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
        ...
        self.kl_criterion = nn.KLDivLoss(reduction="none", log_target=True)
        self.kd_alpha=kd_alpha

    def kl_loss(self, mlm_lprobs, lprobs, target, ignore_index=None, reduce=True):
        kl_loss = self.kl_criterion(lprobs, mlm_lprobs)
        if ignore_index is not None:
            pad_mask = target.eq(ignore_index).unsqueeze(-1).expand(-1, mlm_lprobs.size(-1))
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if sample["suggestion_type"] == "bi_context":
            mlm_net_output = model.mlm_model(src_tokens=sample["net_input"]["src_tokens"],
                            src_lengths=sample["net_input"]["src_lengths"], 
                            prev_output_tokens=sample["bi_context_net_input"]["bi_context_mlm_input_tokens"],
                            tgt_position_ids = None)

            net_output = model(src_tokens=sample["net_input"]["src_tokens"],
                            src_lengths=sample["net_input"]["src_lengths"], 
        ...
        loss, nll_loss = self.compute_loss(model, mlm_net_output, net_output, sample, reduce=reduce)
    ...
    def compute_loss(self, model, mlm_net_output, net_output, sample, reduce=True):
        mlm_lprobs, mlm_target = self.get_lprobs_and_target(model, mlm_net_output, sample)
        ...
        kl_loss = self.kl_loss(
            mlm_lprobs=mlm_lprobs,
            lprobs=lprobs,
            target=target,
            ignore_index=self.padding_idx,
            reduce=reduce)
        loss = (1-self.kd_alpha) * ce_loss + self.kd_alpha * kl_loss
        return loss, nll_loss
