...
class LabelSmoothedCrossEntropyCriterionConfig(FairseqDataclass):
    ...
    loss_weights: List[float] = field(
        ...
        metadata={"help": "loss_weights for generator and reranker"}
    )
...
@register_criterion(
    "gwlan_electra_loss", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class GWLANElectraLossCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if sample["suggestion_type"] == "bi_context":
            net_output = model(
                ...
                reranker_target = sample["reranker_target"],
        ...
        loss, nll_loss, generator_loss, reranker_loss = self.compute_loss(model, net_output, sample, reduce=reduce, epoch_idx = self.task.epoch_idx)
        
        sample_size = (
            sample["reranker_target"].size(0) if self.sentence_avg else sample["ntokens"]
        )
        logging_output = {
            "loss": loss.data,
            "nll_loss": nll_loss.data,
            "ntokens": sample["ntokens"],
            "nsentences": sample["reranker_target"].size(0),
            "sample_size": sample_size,
            "generator_loss": generator_loss.data,
            "reranker_loss": reranker_loss.data
        }
        
        if self.report_accuracy:
            # * accuaracy for generator
            generator_n_correct, generator_total = self.compute_generator_accuracy(model, net_output[0], sample)
            logging_output["g_n_correct"] = utils.item(generator_n_correct.data)
            logging_output["g_total"] = utils.item(generator_total.data)

            # * accuracy for reranker
            reranker_n_correct, reranker_total = self.compute_reranker_accuracy(model, net_output[1], sample)
            logging_output["r_n_correct"] = utils.item(reranker_n_correct.data)
            logging_output["r_total"] = utils.item(reranker_total.data)
    ...
    def eval_forward(self, model, sample, reduce=True):
        ...
        if self.eval_type == "inner_product":
            ...
            # * single generator preformance
            if hasattr(model.cfg, "electra_cut_off") and model.cfg.electra_cut_off == True:
                if self.task.epoch_idx >= model.cfg.electra_cut_off_epoch:
                    labels, preds = self.task.generate([model.pretrained_generator], sample)
                else:
                    labels, preds = self.task.generate([model.generator], sample)
            else:
                labels, preds = self.task.generate([model.generator], sample)
            g_n_correct = sum(labels == preds)
            g_total = preds.shape[0]

            # * generator + reranker performance
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, epoch_idx=None):
        generator_output, reranker_output = net_output

        if hasattr(model.cfg, "electra_cut_off") and model.cfg.electra_cut_off == True:
            if epoch_idx >= model.cfg.electra_cut_off_epoch:
                generator_loss, generator_nll_loss = torch.tensor(0, device=sample["generator_target"].device), torch.tensor(0, device=sample["generator_target"].device)
            else:
                generator_loss, generator_nll_loss = self.compute_generator_loss(model, generator_output, sample, reduce)
        else:
            generator_loss, generator_nll_loss = self.compute_generator_loss(model, generator_output, sample, reduce)

        reranker_loss, reranker_nll_loss = self.compute_reranker_loss(model, reranker_output, sample, reduce)
        
        loss = generator_loss * self.loss_weights[0] + reranker_loss * self.loss_weights[1]
        nll_loss = generator_nll_loss * self.loss_weights[0] + reranker_nll_loss * self.loss_weights[1]
        
        return loss, nll_loss, generator_loss, reranker_loss

    def get_lprobs_and_target(self, model, net_output, sample):
        lprobs = model.generator.get_normalized_probs(net_output, log_probs=True)
        target = model.get_generator_targets(sample, net_output)
    ...
    def compute_generator_loss(self, model, net_output, sample, reduce=True):
    ...
    def get_reranker_logits_and_target(self, model, reranker_output, sample):
        logits = reranker_output[0]
        target = reranker_output[2]
        assert logits.size(0) == target.size(0)
        return logits.view(-1), target

    def compute_reranker_loss(self, model, reranker_output, sample, reduce=True):
        # logits, target：(bsz * top_k) or (bsz * (top_k+1))
        logits, target = self.get_reranker_logits_and_target(model, reranker_output, sample)
        if self.softmax_normalization == False:
            if model.cfg.contrastive_learning == True:
                ...
                last_hidden_states = reranker_output[1]["inner_states"][-1].transpose(0, 1)
                ...
                loss = self.bce_criterion(logits, target.float())
        else:
    ...
    def compute_generator_accuracy(self, model, generator_output, sample):
        lprobs, target = self.get_lprobs_and_target(model, generator_output, sample)
    ...
    def compute_reranker_accuracy(self, model, reranker_output, sample):
        # original formula:
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        reranker_loss_sum = sum(log.get("reranker_loss", 0) for log in logging_outputs)
        metrics.log_scalar(
            "generator_loss", generator_loss_sum / sample_size / math.log(2), sample_size, round=3
        )
        metrics.log_scalar(
            "reranker_loss", reranker_loss_sum / sample_size / math.log(2), sample_size, round=3
        )
        ...
        # * add generator & reranker accuracy
        generator_total = utils.item(sum(log.get("g_total", 0) for log in logging_outputs))
        if generator_total > 0:
            metrics.log_scalar("g_total", generator_total)
            generator_n_correct = utils.item(
                sum(log.get("g_n_correct", 0) for log in logging_outputs)
            )
            metrics.log_scalar("g_n_correct", generator_n_correct)
            metrics.log_derived(
                "g_accuracy",
                lambda meters: round(
                    meters["g_n_correct"].sum * 100.0 / meters["g_total"].sum, 3
                )
                if meters["g_total"].sum > 0
                else float("nan"),
            )

        reranker_total = utils.item(sum(log.get("r_total", 0) for log in logging_outputs))
        if reranker_total > 0:
            metrics.log_scalar("r_total", reranker_total)
            reranker_n_correct = utils.item(
                sum(log.get("r_n_correct", 0) for log in logging_outputs)
            )
            metrics.log_scalar("r_n_correct", reranker_n_correct)
            metrics.log_derived(
                "r_accuracy",
                lambda meters: round(
                    meters["r_n_correct"].sum * 100.0 / meters["r_total"].sum, 3
                )
                if meters["r_total"].sum > 0
                else float("nan"),
            )
