...
@register_criterion(
    "gwlan_tag_binary_cross_entropy", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class GWLANTagBinaryCrossEntropyCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if sample["suggestion_type"] == "bi_context":
            net_output = model(
                ...
                generator_bi_context_position = sample["bi_context_net_input"]["generator_bi_context_position"],
                generator_bi_context_input_tokens = sample["bi_context_net_input"]["generator_bi_context_input_tokens"],
                generator_target_position = sample["generator_target_position"]
            )
    ...
    def eval_forward(self, model, sample, reduce=True):
        ...
        if self.eval_type == "inner_product":
            ...
            n_correct, total = self.task.rerank_tag_generate([model], sample)[:2]
        else:
