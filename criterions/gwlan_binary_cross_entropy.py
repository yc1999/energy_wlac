...
@register_criterion(
    "gwlan_binary_cross_entropy", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class GWLANBinaryCrossEntropyCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if sample["suggestion_type"] == "bi_context":
            ...
            net_output = model(
                src_tokens=sample["net_input"]["src_tokens"],
                src_lengths=sample["net_input"]["src_lengths"],
                prev_output_tokens=sample["ts_net_input"]["ts_input_tokens"],
                tgt_position_ids = sample["ts_net_input"]["ts_position"],
                target_position = sample["target_position"],
                target = sample["target"],
                type_masks = sample["type_masks"],
                tgt_dict = self.task.target_dictionary,
            ...
            net_output = model(
                src_tokens=sample["net_input"]["src_tokens"],
                src_lengths=sample["net_input"]["src_lengths"],
                prev_output_tokens=sample["imt_net_input"]["imt_input_tokens"],
                tgt_position_ids = sample["imt_net_input"]["imt_position"],
                target_position = sample["target_position"],
                target = sample["target"],
                type_masks = sample["type_masks"],
                tgt_dict = self.task.target_dictionary,
